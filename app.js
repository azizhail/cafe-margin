const config={url:'https://wdzgwaeypxvjizageueu.supabase.co',key:'sb_publishable_jE3dU62W7kl2ffOBf0qMqg_Pb3wvOew'};const platformAdminEmails=['erthcafe11@gmail.com'];const isPlatformAdmin=()=>platformAdminEmails.includes(String(session?.user?.email||'').trim().toLowerCase());
...
function forgotPasswordView(){const limitKey='cafemargin-recovery-attempts';const readAttempts=email=>{const stored=JSON.parse(localStorage.getItem(`${limitKey}-${email}`)||'{}');return stored.startedAt&&Date.now()-stored.startedAt<86400000?stored.count||0:0};app.innerHTML='<main class="auth"><form class="auth-card" id="forgot-form"><div class="logo"><b>cm</b><strong>CafeMargin</strong></div><h1>استعادة كلمة المرور</h1><p class="muted">أدخل بريدك الإلكتروني وسيتم إرسال رابط استعادة كلمة المرور إليه.</p><label>البريد الإلكتروني<input id="forgot-email" type="email" required></label><p id="forgot-error" class="error"></p><p id="forgot-success" style="color:var(--mint);font-size:13px;margin:8px 0;display:none;"></p><small id="forgot-count" class="muted">أدخل بريدك لمعرفة المحاولات المتبقية</small><button class="primary" id="btn-submit-forgot">إرسال رابط الاستعادة</button><button type="button" id="btn-to-login" style="background:none;border:none;color:var(--teal);margin-top:12px;font-size:13px;cursor:pointer">العودة لتسجيل الدخول</button></form></main>';const emailInput=document.querySelector('#forgot-email');const countEl=document.querySelector('#forgot-count');const updateCount=()=>{const email=emailInput.value.trim().toLowerCase();countEl.textContent=email?`المتاح: ${Math.max(0,10-readAttempts(email))} من 10 محاولات خلال 24 ساعة`:'أدخل بريدك لمعرفة المحاولات المتبقية'};emailInput.oninput=updateCount;document.querySelector('#btn-to-login').onclick=loginView;document.querySelector('#forgot-form').onsubmit=async event=>{event.preventDefault();const errorEl=document.querySelector('#forgot-error');const successEl=document.querySelector('#forgot-success');const email=emailInput.value.trim().toLowerCase();const btn=document.querySelector('#btn-submit-forgot');const current=JSON.parse(localStorage.getItem(`${limitKey}-${email}`)||'{}');const currentAttempts=readAttempts(email);if(currentAttempts>=10){errorEl.textContent='تم بلوغ الحد الأقصى: 10 محاولات خلال 24 ساعة.';return}errorEl.textContent='';successEl.style.display='none';btn.disabled=true;btn.textContent='جاري الإرسال...';try{const response=await fetch(`${config.url}/auth/v1/recover`,{method:'POST',headers:{apikey:config.key,'Content-Type':'application/json'},body:JSON.stringify({email,redirect_to:'https://azizhail.github.io/cafe-margin/index.html'})});if(!response.ok){const errData=await response.json().catch(()=>({}));throw new Error(errData.msg||errData.error_description||'تعذر إرسال رابط الاستعادة، تأكد من صحة البريد')}const next={startedAt:current.startedAt||Date.now(),count:currentAttempts+1};localStorage.setItem(`${limitKey}-${email}`,JSON.stringify(next));successEl.textContent='تم إرسال رابط استعادة كلمة المرور إلى بريدك الإلكتروني بنجاح! تحقق من صندوق الوارد والبريد المهمل (Spam).';successEl.style.display='block';updateCount()}catch(err){if(err.message.includes('email rate limit exceeded')){errorEl.className='notice';errorEl.textContent='تم الوصول إلى حد إرسال البريد مؤقتًا من مزود الخدمة. انتظر قليلًا ثم أعد المحاولة.';btn.disabled=true;btn.textContent='الإرسال متوقف مؤقتًا';return}errorEl.className='error';errorEl.textContent='تعذر إرسال رسالة الاستعادة، حاول لاحقًا.'}finally{if(!btn.disabled)btn.textContent='إرسال رابط الاستعادة'}}}
...
function activate(view){document.querySelectorAll('.view').forEach(section=>section.classList.toggle('active',section.id===view));document.querySelectorAll('[data-view]').forEach(button=>button.classList.toggle('active',button.dataset.view===view));document.querySelector('#title').textContent={pos:'المبيعات',products:'المنتجات',inventory:'المخزون',reports:'التقارير',assistant:'المستشار',team:'إضافة موظف',printer:'إعداد الطابعة'}[view]||'المبيعات';const renderers={pos:renderPos,products:renderProducts,inventory:renderInventory,reports:renderReports,assistant:renderAssistant,team:renderTeam,printer:renderPrinter};if(renderers[view]){try{renderers[view]()}catch(error){const section=document.querySelector(`#${view}`);if(section)section.innerHTML=`<div class="panel view-error"><h2>تعذر تحميل القسم</h2><p class="muted">${error.message||'حدث خطأ غير متوقع'}</p><button class="primary" type="button" onclick="activate('${view}')">إعادة المحاولة</button></div>`}}}
...
function recoveryToken(){const hashParams=new URLSearchParams(location.hash.slice(1));const queryParams=new URLSearchParams(location.search);const type=hashParams.get('type')||queryParams.get('type');return type==='recovery'?(hashParams.get('access_token')||queryParams.get('access_token')):null}
function employeeEmail(username){return `${username.trim().toLowerCase()}@cafemargin.local`}
async function login(username,password){const email=username.includes('@')?username.trim().toLowerCase():employeeEmail(username);const response=await fetch(`${config.url}/auth/v1/token?grant_type=password`,{method:'POST',headers:{apikey:config.key,'Content-Type':'application/json'},body:JSON.stringify({email,password})});if(!response.ok)throw new Error('اسم المستخدم أو كلمة المرور غير صحيحة');session=await response.json();localStorage.setItem('cafemargin-session',JSON.stringify(session));const members=await api(`shop_members?user_id=eq.${session.user.id}&select=shop_id,role&limit=1`);if(!members?.[0])throw new Error('لا يوجد محل مرتبط بهذا الحساب');role=members[0].role;const shops=await api(`shops?id=eq.${members[0].shop_id}&select=id,name&limit=1`);shop=shops?.[0];products=await api(`products?shop_id=eq.${shop.id}&select=*`)||[]}
function loginView(){app.innerHTML='<main class="auth"><form class="auth-card" id="login-form"><div class="logo"><b>cm</b><strong>CafeMargin</strong></div><h1>تسجيل الدخول</h1><p class="muted">استخدم اسم المستخدم وكلمة المرور.</p><label>اسم المستخدم<input id="login-email" type="text" autocomplete="username" required></label><label>كلمة المرور<input id="login-password" type="password" autocomplete="current-password" required></label><p id="login-error" class="error"></p><button class="primary">دخول</button><button type="button" id="btn-forgot-password" style="background:none;border:none;color:var(--ink-muted);font-size:13px;cursor:pointer;text-decoration:underline">نسيت كلمة المرور؟ استعادة عبر الإيميل</button><button type="button" id="btn-to-signup" style="background:none;border:none;color:var(--teal);font-size:13px;cursor:pointer">ليس لديك حساب؟ إنشاء حساب مقهى جديد</button></form></main>';document.querySelector('#login-form').onsubmit=async event=>{event.preventDefault();try{await login(document.querySelector('#login-email').value,document.querySelector('#login-password').value);render()}catch(error){document.querySelector('#login-error').textContent=error.message}};document.querySelector('#btn-to-signup').onclick=signupView;document.querySelector('#btn-forgot-password').onclick=forgotPasswordView}
function renderTeam(){document.querySelector('#team').innerHTML='<div class="panel"><div class="toolbar"><div><small class="muted">إدارة فريق المقهى</small><h2>إضافة موظف</h2></div></div><p class="muted">أنشئ اسم مستخدم وكلمة مرور للموظف واختر صلاحية واحدة.</p><form id="employee-form"><label>اسم المستخدم<input id="employee-username" pattern="[A-Za-z0-9_.-]+" required></label><label>اسم الموظف<input id="employee-name" required></label><label>كلمة المرور<input id="employee-password" type="password" minlength="6" required></label><label>الصلاحية<select id="employee-role"><option value="sales">المبيعات فقط</option><option value="product_editor">إضافة الأصناف والتعديل عليها فقط</option></select></label><button class="primary">إنشاء الموظف</button><p id="employee-message" class="error"></p></form><div id="employee-list" class="employee-list"></div></div>';const draw=async()=>{const members=await api(`shop_members?shop_id=eq.${shop.id}&select=user_id,role`)||[];document.querySelector('#employee-list').innerHTML=members.filter(item=>['sales','product_editor'].includes(item.role)).map(item=>`<div class="employee-row"><span>موظف</span><b>${item.role==='sales'?'المبيعات':'إدارة الأصناف'}</b></div>`).join('')||'<p class="empty">لا يوجد موظفون بعد.</p>'};document.querySelector('#employee-form').onsubmit=async event=>{event.preventDefault();const message=document.querySelector('#employee-message');const username=document.querySelector('#employee-username').value.trim().toLowerCase();try{const response=await fetch(`${config.url}/auth/v1/signup`,{method:'POST',headers:{apikey:config.key,'Content-Type':'application/json'},body:JSON.stringify({email:employeeEmail(username),password:document.querySelector('#employee-password').value,data:{full_name:document.querySelector('#employee-name').value.trim(),username}})});const result=await response.json();if(!response.ok)throw new Error(result.msg||result.error_description||'تعذر إنشاء الموظف');if(!result.user?.id)throw new Error('لم يتم إنشاء حساب الموظف');await api('shop_members',{method:'POST',body:JSON.stringify({shop_id:shop.id,user_id:result.user.id,role:document.querySelector('#employee-role').value})});message.className='success';message.textContent='تم إنشاء الموظف بنجاح دون إرسال بريد.';event.target.reset();draw()}catch(error){message.className='error';message.textContent=error.message}};draw()}
function shell(){localStorage.setItem('cafemargin-active-shop',shop.id);const canSell=['owner','manager','sales'].includes(role);const canProducts=['owner','manager','product_editor'].includes(role);const canManageTeam=['owner','manager'].includes(role);const nav=`<button data-view="pos">▣ المبيعات</button>${canProducts?'<button data-view="products">◈ المنتجات</button>':''}${canManageTeam?'<button data-view="team">♙ إضافة موظف</button>':''}${canManageTeam?'<button data-view="inventory">◌ المخزون</button><button data-view="reports">▤ التقارير</button><button data-view="assistant">✦ المستشار</button>':''}`;app.innerHTML=`<div class="layout"><aside class="sidebar"><div class="logo"><b>cm</b><strong>CafeMargin</strong></div><div class="branch"><small>المحل النشط</small><strong>${shop.name}</strong></div><nav class="nav">${nav}</nav><button class="printer-link" data-view="printer">▤ إعداد الطابعة</button><div class="profile"><strong>${session.user.user_metadata?.username||session.user.email}</strong><small>${role==='sales'?'مبيعات':role==='product_editor'?'إدارة الأصناف':'مدير المقهى'}</small></div><button class="logout" id="logout">تسجيل الخروج</button></aside><main class="content"><header class="topbar"><div><small class="muted">نظام إدارة المقهى</small><h1 id="title">المبيعات</h1></div><span class="muted">${new Date().toLocaleDateString('ar-SA')}</span></header><section id="pos" class="view active"></section><section id="products" class="view"></section><section id="inventory" class="view"></section><section id="reports" class="view"></section><section id="assistant" class="view"></section><section id="team" class="view"></section><section id="printer" class="view"></section></main></div>`;enhanceSidebar();document.querySelector('#logout').onclick=()=>{localStorage.removeItem('cafemargin-session');session=null;loginView()};app.onclick=event=>{const button=event.target.closest('[data-view]');if(button&&app.contains(button)){event.preventDefault();event.stopPropagation();activate(button.dataset.view)}};if(canSell)renderPos();if(canProducts)renderProducts();if(canManageTeam){renderTeam();renderInventory();renderReports();renderAssistant()}renderPrinter()}
function enhanceSidebar(){const nav=document.querySelector('.nav');if(!nav||nav.dataset.grouped)return;const groups=[['المبيعات والعملاء',['pos']],['المنتجات والمخزون',['products','inventory']],['التقارير والتحليلات',['reports','assistant']],['إدارة الفريق',['team']]];const buttons={};nav.querySelectorAll('[data-view]').forEach(button=>{buttons[button.dataset.view]=button});nav.innerHTML='';groups.forEach(([label,views])=>{const available=views.filter(view=>buttons[view]);if(!available.length)return;const group=document.createElement('div');group.className='nav-group';const heading=document.createElement('div');heading.className='nav-group-title';heading.textContent=label;group.appendChild(heading);available.forEach(view=>group.appendChild(buttons[view]));nav.appendChild(group)});nav.dataset.grouped='true'}

const recoveryAccessToken=recoveryToken();
if(recoveryAccessToken)resetPasswordView(recoveryAccessToken);else start();
