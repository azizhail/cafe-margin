...
function forgotPasswordView(){const limitKey='cafemargin-recovery-attempts';const stored=JSON.parse(localStorage.getItem(limitKey)||'{}');const now=Date.now();const attempts=stored.startedAt&&now-stored.startedAt<86400000?stored.count||0:0;app.innerHTML='<main class="auth"><form class="auth-card" id="forgot-form"><div class="logo"><b>cm</b><strong>CafeMargin</strong></div><h1>استعادة كلمة المرور</h1><p class="muted">أدخل بريدك الإلكتروني وسيتم إرسال رابط استعادة كلمة المرور إليه.</p><label>البريد الإلكتروني<input id="forgot-email" type="email" required></label><p id="forgot-error" class="error"></p><p id="forgot-success" style="color:var(--mint);font-size:13px;margin:8px 0;display:none;"></p><small id="forgot-count" class="muted">المتاح: ${Math.max(0,10-attempts)} من 10 محاولات خلال 24 ساعة</small><button class="primary" id="btn-submit-forgot">إرسال رابط الاستعادة</button><button type="button" id="btn-to-login" style="background:none;border:none;color:var(--teal);margin-top:12px;font-size:13px;cursor:pointer">العودة لتسجيل الدخول</button></form></main>';document.querySelector('#btn-to-login').onclick=loginView;document.querySelector('#forgot-form').onsubmit=async event=>{event.preventDefault();const errorEl=document.querySelector('#forgot-error');const successEl=document.querySelector('#forgot-success');const countEl=document.querySelector('#forgot-count');const email=document.querySelector('#forgot-email').value.trim().toLowerCase();const btn=document.querySelector('#btn-submit-forgot');const current=JSON.parse(localStorage.getItem(limitKey)||'{}');const currentAttempts=current.startedAt&&Date.now()-current.startedAt<86400000?current.count||0:0;if(currentAttempts>=10){errorEl.textContent='تم بلوغ الحد الأقصى: 10 محاولات خلال 24 ساعة.';return}errorEl.textContent='';successEl.style.display='none';btn.disabled=true;btn.textContent='جاري الإرسال...';try{const response=await fetch(`${config.url}/auth/v1/recover`,{method:'POST',headers:{apikey:config.key,'Content-Type':'application/json'},body:JSON.stringify({email,redirect_to:'https://azizhail.github.io/cafe-margin/'})});if(!response.ok){const errData=await response.json().catch(()=>({}));throw new Error(errData.msg||errData.error_description||'تعذر إرسال رابط الاستعادة، تأكد من صحة البريد')}const next={startedAt:current.startedAt||Date.now(),count:currentAttempts+1};localStorage.setItem(limitKey,JSON.stringify(next));successEl.textContent='تم إرسال رابط استعادة كلمة المرور إلى بريدك الإلكتروني بنجاح! تحقق من صندوق الوارد والبريد المهمل (Spam).';successEl.style.display='block';countEl.textContent=`المتاح: ${Math.max(0,10-next.count)} من 10 محاولات خلال 24 ساعة`}catch(err){errorEl.textContent=err.message}finally{btn.disabled=false;btn.textContent='إرسال رابط الاستعادة'}}}
