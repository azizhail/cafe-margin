...
function loginView(){app.innerHTML='<main class="auth"><form class="auth-card" id="login-form"><div class="logo"><b>cm</b><strong>CafeMargin</strong></div><h1>تسجيل الدخول</h1><p class="muted">استخدم البريد الإلكتروني وكلمة المرور السحابية.</p><label>البريد الإلكتروني<input id="login-email" type="email" required></label><label>كلمة المرور<input id="login-password" type="password" required></label><p id="login-error" class="error"></p><button class="primary">دخول</button><button type="button" id="btn-to-signup" class="primary" style="background:none;color:var(--teal);margin-top:12px;font-size:13px">ليس لديك حساب؟ إنشاء حساب مقهى جديد</button></form></main>';document.querySelector('#login-form').onsubmit=async event=>{event.preventDefault();try{await login(document.querySelector('#login-email').value.trim(),document.querySelector('#login-password').value);render()}catch(error){document.querySelector('#login-error').textContent=error.message}};document.querySelector('#btn-to-signup').onclick=signupView}
